...
parser.add_argument(
    dest="filename", help="Custom build input file (.yml)", metavar="FILE.yml"
)
...
def verifyBindings(bindings):
    for binding in bindings:
        if not verifyBinding(binding):
            raise Exception(
                "Requested binding " + json.dumps(binding) + " does not exist!"
            )
...
for dirpath, dirnames, filenames in os.walk(libraryBasePath + "/bindings"):
    for item in filenames:
        if item.endswith(".d.ts.json") and shouldProcessSymbol(
            item[:-10],
            list(
                chain(
...
def runBuild(build):
    def getAdditionalBindCodeO():
        if "additionalBindCode" in build:
            ...
            additionalBindCodeFileName = (
                libraryBasePath + "/additionalBindCode/" + build["name"] + ".cpp"
            )
    ...
    for dirpath, dirnames, filenames in os.walk(libraryBasePath + "/bindings"):
        for item in filenames:
            if item.endswith(".cpp.o") and shouldProcessSymbol(
                item[:-6], build["bindings"]
            ):
...
if buildConfig["generateTypescriptDefinitions"]:
    ...
    with open(
        os.getcwd()
        + "/"
        + os.path.splitext(buildConfig["mainBuild"]["name"])[0]
        + ".d.ts",
        "w",
    ) as typescriptDefinitionsFile:
